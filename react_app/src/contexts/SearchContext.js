...
export default function useSearch() {
  ...
  function updateStateProjects(value) {
    ...
    if (!searchWord) return setProjectsData(dataProjects);
    const newFilter = dataProjects.filter((value) =>
  ...
  function updateStateCases(value) {
    const searchWord = value;
    setWordValue(searchWord);
    if (!searchWord) return setCasesData(dataCases);
    const newFilter = dataCases.filter((item) =>
